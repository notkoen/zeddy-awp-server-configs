...
public void OnMapStart()
{
	//Find player spawns
	char classname[64];
	g_iTSpawnPointsCount = 0;
	g_iCTSpawnPointsCount = 0;
	...
	while((entity = FindEntityByClassname(entity, "info_player_*")) != INVALID_ENT_REFERENCE)
	{
		...
		{
			GetEntPropVector(entity, Prop_Send, "m_vecOrigin", g_fTSpawnPoints[g_iTSpawnPointsCount][0]);
			GetEntPropVector(entity, Prop_Send, "m_angRotation", g_fTSpawnPoints[g_iTSpawnPointsCount][1]);

			g_iTSpawnPointsCount++;
			if (g_iTSpawnPointsCount >= MAX_PLAYER_SPAWNS)
			{
				break;
			}
		}
		else if(StrEqual(classname, "info_player_counterterrorist", true))
		{
			GetEntPropVector(entity, Prop_Send, "m_vecOrigin", g_fCTSpawnPoints[g_iCTSpawnPointsCount][0]);
			GetEntPropVector(entity, Prop_Send, "m_angRotation", g_fCTSpawnPoints[g_iCTSpawnPointsCount][1]);

			g_iCTSpawnPointsCount++;
			if (g_iCTSpawnPointsCount >= MAX_PLAYER_SPAWNS)
			{
				break;
			}
		}
	}
	if(g_iTSpawnPointsCount==0) LogError("Failed to find any spawn points for terrorist team.");
	if(g_iCTSpawnPointsCount==0) LogError("Failed to find any spawn points for counterterrorist team.");
}
...
//Force teleport every player spawn to their team spawn point
void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));

	int team = GetClientTeam(client);
	if(team==CS_TEAM_T && g_iTSpawnPointsCount != 0)
	{
		int select = GetRandomInt(0, g_iTSpawnPointsCount - 1);
		TeleportEntity(client, g_fTSpawnPoints[select][0], g_fTSpawnPoints[select][1], NULL_VECTOR);
	}
	else if(team==CS_TEAM_CT && g_iCTSpawnPointsCount != 0)
	{
		int select = GetRandomInt(0, g_iCTSpawnPointsCount - 1);
		TeleportEntity(client, g_fCTSpawnPoints[select][0], g_fCTSpawnPoints[select][1], NULL_VECTOR);
	}
}
